...
PYTHONPATH="${upperdir}"/python "$hesin_script" -p "OA" -r "$hesin_release" --icd9 "$tempfiles[tmp_icd9_incl]" --icd10 "$tempfiles[tmp_icd10_incl]" -o "$tempfiles[tmp_inclusion_icd_out]" 2> >(tee -a "$logfile" >&2)
...
PYTHONPATH="${upperdir}"/python "$hesin_script" -p "OA" -r "$hesin_release" --oper4 "$tempfiles[tmp_opcs4_incl]" -o "$tempfiles[tmp_inclusion_opcs4_out]" 2> >(tee -a "$logfile" >&2)
