...
PYTHONPATH="${upperdir}"/python "$hesin_script" -p "OA" -r "$hesin_release" --oper4 "${tempfiles[tmp_opcs4_incl]}" -o "${tempfiles[tmp_inclusion_opcs4_out]}" > >(tee -a "$logfile") 2> >(tee -a "$logfile" >&2)
