...
join --header -1 1 -2 1 -e NA -a 1 -a 2 -o ${main_str},${diag_str} <(cat <(head -n 1 $main_fname| sed 's/\t/\./') <(tail -n +2 $main_fname| sed 's/\t/\./'| sort -k1,1)) <(cat <(head -n 1 $diag_fname|sed 's/level/diag_level/'|sed 's/arr_index/diag_arr_index/'| sed 's/\t/\./') <(tail -n +2 $diag_fname| sed 's/\t/\./'| sort -k1,1))| tr ' ' '\t' | join --header -1 1 -2 1 -e NA -a 1 -a 2 -o ${join_str},${oper_str} - <(cat <(head -n 1 $oper_fname|sed 's/level/oper_level/'|sed 's/arr_index/oper_arr_index/'|sed 's/\t/\./') <(tail -n +2 $oper_fname| sed 's/\t/\./'| sort -k1,1)) | tr '\.' ',' | tr ' ' ','| awk -v d=$datestr -v r=$release 'BEGIN{FS=",";OFS=",";}{if (NR==1){print $0,"CREATED","RELEASE";}else{print $0,d,r;}}'
